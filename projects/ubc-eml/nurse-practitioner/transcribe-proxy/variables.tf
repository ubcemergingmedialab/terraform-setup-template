...
variable "transcribe_shared_secret" {
  type        = string
  description = "Optional shared secret the game sends in the x-transcribe-secret header on WebSocket connect. When set, the container rejects connections without it. Leave empty to disable (rely on allowed_cidr_blocks instead). Set as a sensitive HCP workspace variable, never in tfvars."
  default     = ""
  sensitive   = true
}

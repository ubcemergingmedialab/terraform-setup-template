locals {
  ...
  # Base env for the Node.js proxy; add the shared secret only when configured so
  # the container gates connections on the x-transcribe-secret header.
  container_environment = merge(
    ...
    var.transcribe_shared_secret != "" ? { TRANSCRIBE_SHARED_SECRET = var.transcribe_shared_secret } : {},
  )
}
...
# Replaces the EC2 "MOOT-API" websocket proxy for the STT path: Unreal keeps its
# DXL WebSocket client and points it here; the container relays PCM audio to
# Amazon Transcribe Streaming and returns transcripts as JSON.
module "transcribe_proxy" {
  ...
  # IAM permissions for the task to call Transcribe (same shape as episode).

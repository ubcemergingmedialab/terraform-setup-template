...
// Optional shared secret. When set, a connecting client must present it either in
// the x-transcribe-secret header or a ?secret= query param, else the connection is
// rejected. Leave unset to rely on ALB CIDR restrictions instead.
const SHARED_SECRET = process.env.TRANSCRIBE_SHARED_SECRET || '';

// Create WebSocket server. When a secret is configured, verify it during the
// upgrade handshake so unauthorized clients never open a socket / Transcribe stream.
const wss = new WebSocket.Server({
    port: PORT,
    verifyClient: (info) => {
        if (!SHARED_SECRET) return true;
        const provided = extractSecret(info.req);
        return timingSafeEqual(provided, SHARED_SECRET);
    },
});

function extractSecret(req) {
    const headerSecret = req.headers['x-transcribe-secret'];
...
console.log(`  Shared secret: ${SHARED_SECRET ? 'ENABLED' : 'disabled'}`);
...
wss.on('connection', async (ws, req) => {
    ...
    try {
        ...
        // Frame format: [4-byte little-endian sample rate][PCM16 mono audio].
